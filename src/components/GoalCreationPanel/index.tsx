...
const GoalCreationPanel = ({state, closeFunction}:{ state: boolean, closeFunction: React.Dispatch<React.SetStateAction<boolean>>}) => {
    ...
    return state ? (
        <div onClick={()=>closeFunction(false)} className='absolute flex justify-center items-center z-10 bg-black/60 h-full w-full'>
            <form className='w-[35vw] z-20 h-[80vh] border-r-[24px] min-w-[400px] bg-slate-100 rounded
            border-teal-500 shadow-md flex flex-col py-12 px-4 gap-y-4'>

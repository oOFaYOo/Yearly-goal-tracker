...
class ApiClient implements IApiClient{
    ...
    async signIn(login: string, password: string) : Promise<IApiClientResult<string>> {
        ...
        // } else return {status: false, message: response.data.message}
    ...
    async signUp(login: string, password: string): Promise<IApiClientResult<void>> {
        const response = await axios.post(
        ...
        // if (response.status === 200) {
        //     return {status: false, message:'Successfully registered'}
        // } else return {status: false, message: response.data.message}
        return {
            isSuccessful: response.status === 200,
            isAuthorized: response.status !== 401,
